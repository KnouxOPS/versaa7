...
export function AIModelsManager() {
  const [models, setModels] = useState<AIModel[]>([
    {
      ...
      type: "diffusion",
      size: "4.2 GB",
      description: "توليد صور واقعية/تجريدية من توصيف نصي متقدم",
      capabilities: [
        "Text-to-Image",
        "Image-to-Image",
        "Inpainting",
      ...
      status: "installed",
      performance: { speed: 95, quality: 98, memoryUsage: "4.2GB" },
      requirements: { gpu: true, minRam: "8GB", diskSpace: "5GB" },
    },
    {
      id: "rv51",
  ...
  const handleUninstall = (modelId: string) => {
    setModels(
      models.map((model) =>
        model.id === modelId ? { ...model, status: "available" } : model,
      ),
    );
  };
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-300 text-sm">
              إدارة وتحميل النماذج التوليدية المتقدمة | Advanced AI Models
              Management
            </p>
          </div>
        ...
        <div className="bg-gray-800/50 rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-cyan-400">
              {installedModels
                .reduce((total, model) => total + parseFloat(model.size), 0)
                .toFixed(1)}{" "}
              GB / 50 GB
            </span>
          </div>
          <Progress
            value={
              (installedModels.reduce(
                (total, model) => total + parseFloat(model.size),
                0,
              ) /
                50) *
              100
          ...
          >
            ...
            <div className="text-xs text-gray-400 mb-4 space-y-1">
              <div className="flex items-center gap-2">
                <i
                  className={`fas ${model.requirements.gpu ? "fa-microchip text-green-400" : "fa-microchip text-gray-500"}`}
                ></i>
                <span>
                  GPU: {model.requirements.gpu ? "Required" : "Optional"}
                </span>
              </div>
              <div className="flex items-center gap-2">
