    ...
    switch (exportFormat) {
      ...
      case "json":
        const tokens = {
          ...
          spacing: Object.fromEntries(spacingTokens.map(t => [t.name, t.value])),
          typography: Object.fromEntries(typographyTokens.map(t => [t.name, {
            fontSize: t.fontSize,
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-400">
              {lang === "ar" ? "النظام الأساسي للألوان والخطوط والمسافات" : "Foundation system for colors, typography, and spacing"}
            </p>
          </div>
      ...
      <div className="flex-1">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full">
          <TabsList className="grid w-full grid-cols-4">
            <TabsTrigger value="colors">{lang === "ar" ? "الألوان" : "Colors"}</TabsTrigger>
            <TabsTrigger value="typography">{lang === "ar" ? "الخطوط" : "Typography"}</TabsTrigger>
            <TabsTrigger value="spacing">{lang === "ar" ? "المسافات" : "Spacing"}</TabsTrigger>
            <TabsTrigger value="effects">{lang === "ar" ? "التأثيرات" : "Effects"}</TabsTrigger>
          </TabsList>

          <TabsContent value="colors" className="mt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {colorTokens.map((token) => (
                <Card key={token.name} className="glass-light p-4 hover:border-cyan-400/50 transition-all duration-300">
                  <div
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <code className="text-cyan-400 text-sm">{token.name}</code>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400 text-sm">Size:</span>
                        <code className="text-cyan-400 text-sm">{token.fontSize}</code>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400 text-sm">Weight:</span>
                        <code className="text-cyan-400 text-sm">{token.fontWeight}</code>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400 text-sm">Line Height:</span>
                        <code className="text-cyan-400 text-sm">{token.lineHeight}</code>
                      </div>
                    </div>
                  </div>
                </Card>
                ...
                <Card key={token.name} className="glass-light p-6">
                  <div className="space-y-4">
                    ...
                    <div 
                      ...
                      style={{
                        boxShadow: token.name.includes('glow') ? token.value : 
                                  token.name.includes('shadow') ? token.value : 'none',
                        filter: token.name.includes('blur') ? token.value : 'none'
                      }}
                    >
